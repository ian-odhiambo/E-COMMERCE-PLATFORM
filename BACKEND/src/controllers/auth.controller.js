...
export const logout = async(req, res) => {
    try {
        ...
        if (refreshToken) {
            ...
            // Fixed: Use 'id' instead of 'userId' to match the token payload
...
export const refreshToken = async(req, res) => {
    try {
        ...
        res.cookie("accessToken", accessToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === "production", // Fixed: Changed '==' to '==='
            sameSite: "strict",
            maxAge: 15 * 60 * 1000,
        });

        // Fixed: Changed 'succesfully' to 'successfully'
...
export const getProile = async (req, res) => {
    try {
        res.json(req.user)
    }catch(error){
        res.status(500).json({ message: "Server error", error: error.message })
